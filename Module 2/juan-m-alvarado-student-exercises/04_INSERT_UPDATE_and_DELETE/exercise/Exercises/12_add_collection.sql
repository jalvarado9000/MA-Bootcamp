...
UPDATE movie SET collection_id = (SELECT collection_id FROM collection WHERE collection_name = 'Bill Murray Collection') WHERE movie_id
IN (SELECT A.movie_id FROM movie_actor A JOIN person P ON A.actor_id = P.person_id WHERE P.person_name = 'Bill Murray');