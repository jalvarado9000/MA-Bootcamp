...
function isAdmitted(gpa,satScore,recommendation){
    ...
    let floatGpa = parseFloat(gpa)
    let intSatScore = parseInt(satScore)

    if(floatGpa > 3.0 && recommendation){
        return true
    }
    else if(floatGpa >= 4.0 || intSatScore > 1300){
        return true
    }
    else if(intSatScore > 1200 && recommendation){
        return true
    }
...
function makeNumber(str1,str2){
    let conCatenate1 = str1 + str2
    let result = parseInt(conCatenate1)
    return result
...
function getFullAddressesOfProperties(objects){
    return objects.map(address => address.streetNumber + ' '
+ address.streetName + ' ' + address.streetType + ' ' + address.city + ' ' +
